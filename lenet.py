...
for epoch in range(20):
    ...
    for i in range(images.shape[0] / batch_size):
        ...
        batch_loss += sf.calc_loss(fc_out, np.array(label))
        train_loss += sf.calc_loss(fc_out, np.array(label))
        ...
        if i % 1 == 0:
            fc.backward(lr=learning_rate, weight_decay=0.0004)
            conv2.backward(lr=learning_rate, weight_decay=0.0004)
            conv1.backward(lr=learning_rate, weight_decay=0.0004)
